...
router.get("/authorCountry/:country", (req, res) => {

  // Se filtra la lista de autores por el país especificado en los parámetros de la URL
  const country = data.dataLibrary.authors.filter((country) => {
    return country.country.includes(req.params.country);
  })

  // Se prepara la respuesta que se enviará al cliente
  const response = {
    service: "authors",
    architecture: "microservices",
